...
/**
 * Interface for future Component C historical calculation inputs.
 */
export interface IHistoryScoringInput {
  previousScore: number;
  similarity: number;
  feedbackStatus: 'confirmed' | 'unsure' | 'rejected' | string;
}

/**
 * Future computation utility for Component C (Historical Context).
 * Iterates only previous saved dreams, excluding the current dream.
 */
export function futureComputeComponentC(historyItems: IHistoryScoringInput[]): number {
  ...
  for (const item of historyItems) {
    let feedbackWeight = 0.50;
    if (item.feedbackStatus === 'confirmed') {
      feedbackWeight = 1.0;
    } else if (item.feedbackStatus === 'rejected') {
      feedbackWeight = 0.0;
    }

    const impact = (item.previousScore - 50) * item.similarity * feedbackWeight;
    sumImpact += impact;
  }

  const averageImpact = sumImpact / historyItems.length;
  return Math.min(100, Math.max(0, 50 + averageImpact));
}
 ...
 */
export function getValenceLabel(score: number): string {
