...
import {
  ...
  parseOracleMode,
  parseOracleObjectId,
} from '../../dto/oracleRequest.dto';
import { OracleContractError } from '../../services/oracle.types';
import { createOracleTurnRun } from '../../services/persistence/oracleTurnRunPersistence.service';
import {
  deleteOracleThread,
  getOracleThread,
} from '../../controllers/oracleThread.controller';
import { postOracleTurn } from '../../controllers/oracleTurn.controller';
import { cancelOracleRun } from '../../controllers/oracleRun.controller';
...
// ─── Section A: Strict Feature Flag Tests ────────────────────────────────────
  ...
  test('Section D: Access Scope and Ownership boundaries', () => {
    ...
    const typesPath = path.resolve(__dirname, '../../services/oracle.types.ts');
    ...
    // Read the shared HTTP boundary to prove ownership derives from req.user._id.
    const controllerPath = path.resolve(__dirname, '../../controllers/oracleHttp.controller.ts');
    ...
    const oracleRetrievalPath = path.resolve(
      __dirname,
      '../../services/grounding/oracleGroundingRetrieval.service.ts',
    );
    ...
    const oraclePromptPath = path.resolve(
      __dirname,
      '../../services/grounding/oracleGroundingPrompt.service.ts',
    );
    const oraclePromptContent = fs.readFileSync(oraclePromptPath, 'utf8');
    assertBehavior(
      oraclePromptContent.includes('untrusted_retrieved_content'),
