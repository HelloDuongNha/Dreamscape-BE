...
import type { DocumentResearchProfile, DocumentExtractionPlan } from '../../services/planning/documentResearchProfile.types';
import type { EvidenceBatchPlan } from '../../services/planning/evidenceBatchPlanner.types';
import type { HierarchicalEvidencePlan } from '../../services/planning/hierarchicalEvidencePlanner.types';
import { RuleV3GenerationProvider, ProviderCandidate, RuleV3ProviderInput } from '../../services/providers/ruleV3GenerationProvider.types';
import {
  LIMIT_CANDIDATES,
  LIMIT_EVIDENCE_ITEMS,
  OLLAMA_JSON_SCHEMA
} from '../../services/providers/ruleV3ProviderContract.service';
import { validateProviderResponse } from '../../services/providers/ruleV3ProviderResponseValidator.service';
...
global.fetch = (async (url: string, options: any) => {
  ...
  if (mockFetchResponseFn) {
    return adaptLegacyFixtureResponse(mockFetchResponseFn(url, options), lastRequestBody);
  }
  ...
  return {
    ...
    json: async () => adaptLegacyFixturePayload(mockFetchPayload, lastRequestBody)
  } as any;
}) as any;

// Old fixture quotes are converted at the test boundary; runtime providers only accept evidenceId.
function adaptLegacyFixtureResponse(response: any, requestBody: any) {
  if (!response || typeof response.json !== 'function') return response;
  const readJson = response.json.bind(response);
  return {
    ...response,
    json: async () => adaptLegacyFixturePayload(await readJson(), requestBody)
  };
}

function adaptLegacyFixturePayload(payload: any, requestBody: any) {
  if (!payload || typeof payload !== 'object') return payload;
  const prompt = requestBody?.prompt || requestBody?.contents?.[0]?.parts?.[0]?.text || '';
  const anchors = parseEvidenceAnchors(prompt);
  if (!anchors.length) return payload;

  const responseText = typeof payload.response === 'string'
    ? payload.response
    : payload.candidates?.[0]?.content?.parts?.[0]?.text;
  if (typeof responseText !== 'string') return payload;
  ...
  for (const candidate of parsed.candidates || []) {
    candidate.evidence = (candidate.evidence || []).map((evidence: any) => {
      if (evidence.evidenceId || !evidence.proposedQuote) return evidence;
      const matchingAnchors = anchors.filter(anchor =>
        anchor.chunkId === evidence.chunkId
        && countOccurrences(anchor.exactQuote, evidence.proposedQuote) === 1
      );
      return {
        evidenceId: matchingAnchors.length === 1
          ? matchingAnchors[0].evidenceId
          : `eva_invalid_${String(evidence.chunkId || 'unknown')}`,
  ...
  if (typeof payload.response === 'string') return { ...payload, response: normalized };
  return {
    ...payload,
    candidates: payload.candidates.map((candidate: any, index: number) => index === 0
      ? {
          ...candidate,
          content: {
            ...candidate.content,
            parts: candidate.content.parts.map((part: any, partIndex: number) =>
              partIndex === 0 ? { ...part, text: normalized } : part
            )
          }
        }
      : candidate)
...
function parseEvidenceAnchors(prompt: string) {
  const anchors: Array<{ evidenceId: string; chunkId: string; exactQuote: string }> = [];
  const pattern = /\[EVIDENCE\]\n\[evidenceId\]: ([^\n]+)\n\[chunkId\]: ([^\n]+)\n\[exactQuote\]: ([\s\S]*?)(?=\n\n\[EVIDENCE\]|\n\[DATA_END\])/gu;
  for (const match of prompt.matchAll(pattern)) {
    anchors.push({ evidenceId: match[1], chunkId: match[2], exactQuote: match[3] });
  }
  return anchors;
}
...
async function runTests() {
  ...
  // Test 10: A translated quote cannot resolve to a supplied evidenceId
  {
    resetMocks();
    mockFetchPayload = {
      response: JSON.stringify({
    ...
    assert.strictEqual(result.citationVerifiedCandidates.length, 0);
    assert.strictEqual(result.rejectedCandidates.length, 1);
    assert.strictEqual(result.rejectedCandidates[0].reasonCode, 'evidence_reference_invalid');
    console.log('[PASS] Translated quote cannot resolve to an immutable evidence reference');
  }

  // Test 11: A changed negation cannot resolve to a supplied evidenceId
  {
    resetMocks();
    mockFetchPayload = {
      response: JSON.stringify({
    ...
    assert.strictEqual(result.citationVerifiedCandidates.length, 0);
    assert.strictEqual(result.rejectedCandidates[0].reasonCode, 'evidence_reference_invalid');
    console.log('[PASS] Changed negation cannot resolve to an immutable evidence reference');
  }

  // Test 12: An ambiguous quote hint cannot resolve to one supplied evidenceId
  {
    resetMocks();
    mockFetchPayload = {
      response: JSON.stringify({
    ...
    assert.strictEqual(result.citationVerifiedCandidates.length, 0);
    assert.strictEqual(result.rejectedCandidates[0].reasonCode, 'evidence_reference_invalid');
    console.log('[PASS] Ambiguous quote hint cannot resolve to one immutable evidence reference');
  }

  // Test 13: Evidence outside the work unit cannot resolve to a supplied evidenceId
  {
    resetMocks();
    mockFetchPayload = {
      response: JSON.stringify({
    ...
    assert.strictEqual(result.citationVerifiedCandidates.length, 0);
    assert.strictEqual(result.rejectedCandidates[0].reasonCode, 'evidence_reference_invalid');
    console.log('[PASS] Outside-work-unit evidence cannot resolve to an immutable reference');
  }
  ...
  // Test 15: Unsupported null finding rejected
  {
    ...
    assert.strictEqual(result.citationVerifiedCandidates.length, 0);
    assert.strictEqual(result.rejectedCandidates[0].reasonCode, 'claim_type_evidence_mismatch');
    console.log('[PASS] Unsupported null finding rejected because the evidence reports a positive relation');
  }
  ...
  // Test 16: Duplicate candidate merged
  {
    ...
    assert.strictEqual(result.citationVerifiedCandidates[0].evidence.length, 1);
