...
} from '../../services/evidence/ruleV3CandidateQuality.service';
import {
  hasSpecificRuleCondition,
  hasSpecificRuleLimitation,
} from '../../services/evidence/ruleV3CandidatePolicy.service';
import { pruneUnsupportedSupportingEvidence } from '../../services/evidence/ruleV3EvidenceEntailment.service';
...
const singleCaseNarrative = assessRuleV3CandidateQuality({
  ...base,
  statement: 'Giấc mơ sau cùng của bệnh nhân có liên hệ với trạng thái tâm lý của người đó.',
  claimType: 'theoretical_proposition',
  evidenceInterpretation: 'interpretive',
  subject: 'giấc mơ sau cùng của bệnh nhân',
  outcome: 'trạng thái tâm lý của người đó'
}, [{ stance: 'supports', exactQuote: 'Giấc mơ sau cùng của bệnh nhân có liên hệ với trạng thái tâm lý của người đó.' }]);
assert.equal(singleCaseNarrative.accepted, false);
assert.ok(singleCaseNarrative.reasonCodes.includes('case_specific_narrative'));
...
const generalizedIdentityCheck = assessRuleV3CandidateQuality({
  ...base,
  statement: 'Một nhóm thiểu số tôn giáo được mô tả là thấp kém và phi lý.',
  claimType: 'theoretical_proposition',
  evidenceInterpretation: 'interpretive',
  subject: 'nhóm thiểu số tôn giáo',
  outcome: 'thấp kém và phi lý'
}, [{ stance: 'supports', exactQuote: 'Một nhóm thiểu số tôn giáo được mô tả là thấp kém và phi lý.' }]);
assert.ok(generalizedIdentityCheck.reasonCodes.includes('identity_stereotype'));
