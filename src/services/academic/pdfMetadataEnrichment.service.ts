...
import { resolveSourceImport } from '../sourceImportResolver.service';
import { normalizeDoi } from '../openAccess.service';
...
function isMeaningfulResolverTitle(
  ...
  metadataProvider: string | undefined,
...
export async function enrichPdfMetadata(
  ...
  // 2. Perform text-layer extraction in-memory (already updates pdfPageCount)
  ...
  const detection = detectPdfMetadata(extractedDoc, existingMetaCompare);
  ...
  if (!title || isFilenameFallback(title, originalFileName)) {
    if (isMeaningfulResolverTitle(resolvedMeta?.title, resolvedMeta?.metadataProvider, cleanedIdentifiers)) {
