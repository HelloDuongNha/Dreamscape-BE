...
export function parsePmcHtml(htmlText: string): CanonicalBlocksOutput {
  const blocks: CanonicalBlock[] = [];
  const warnings: string[] = [];
  let title = 'Untitled PMC Article';
  let order = 0;
  ...
  try {
    ...
    // 1. Title Extraction
    const articleTitle = $('.content-title, .title, h1').first().text().trim();
    if (articleTitle) {
      title = articleTitle;
      blocks.push({
        blockType: 'title',
    ...
    const content = $('.ncbi-pmc-body, #mc, article, body').first();
    let currentHeading: string | null = null;

    content.find('h1, h2, h3, h4, p, ul, ol, table, .fig').each((_, el) => {
      const $el = $(el);
      const tagName = el.tagName?.toLowerCase();

      if ($el.closest('.ref-list, footer').length > 0) return;
    ...
    const refContainer = $('.ref-list, .reflist').first();
    if (refContainer.length > 0) {
      ...
      refContainer.find('.ref-item, .ref, li').each((_, ref) => {
    ...
    return {
      title,
      parserEngine: 'PmcParser',
      sourceType: 'publisher_html',
      warnings: [...warnings, `PMC HTML parse failed: ${err.message}`],
      blocks,
      success: false,
      error: err.message
    };
