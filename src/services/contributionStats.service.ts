 ...
 * - Fallback: +10 points
 */
export function calculatePointsForSource(contribution: any): number {
 ...
 */
export function getLevelProgressInfo(approvedCount: number) {
  let currentLevel = 0;
 ...
 */
export async function checkAndAwardLevelAchievements(userId: string, approvedCount: number) {
  const uId = new mongoose.Types.ObjectId(userId);
